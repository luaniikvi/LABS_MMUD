#include <iostream>

// Array containing the MD5 collision Block A
const unsigned char collision_block[128] = {
    
};

int main() {
    // Check byte at index 20 (differing byte between block A and block B: 0x12 for A, 0x07 for B)
    if (collision_block[20] == 0x12) {
        std::cout << "[PROGRAM A] Status: AUTHORIZED. Welcome Admin!" << std::endl;
    } else {
        std::cout << "[PROGRAM A] Status: UNAUTHORIZED. Access Denied." << std::endl;
    }
    return 0;
}
