...
std::vector<uint8_t> read_file(const std::string& path) {
    ...
    if (!file.is_open()) fail_closed("Cannot open file: " + path);
    return std::vector<uint8_t>((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
}

void write_file(const std::string& path, const std::vector<uint8_t>& data) {
    std::ofstream file(path, std::ios::binary);
    if (!file.is_open()) fail_closed("Cannot write to file: " + path);
...
std::vector<uint8_t> string_to_bytes(const std::string& str) {
    ...
    if (wide_len == 0) return std::vector<uint8_t>(str.begin(), str.end());
    ...
    if (utf8_len == 0) return std::vector<uint8_t>(str.begin(), str.end());
...
std::string bytes_to_string(const std::vector<uint8_t>& bytes) {
    ...
    if (wide_len == 0) return std::string(bytes.begin(), bytes.end());
    std::wstring wide_str(wide_len, 0);
    ...
    if (ansi_len == 0) return std::string(bytes.begin(), bytes.end());
