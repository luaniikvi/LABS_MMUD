...
class HashEngine:
    ...
    def hash_text(self, algo, text):
        ...
        rc = self.dll.lab4_hash(algo.encode(), arr, len(data), buf, 512, err)
        if rc != 0:
    ...
    def hash_file(self, algo, filepath, stream=False):
        buf = ctypes.create_string_buffer(512)
        err = ctypes.create_string_buffer(1024)
        rc = self.dll.lab4_hash_file(algo.encode(), filepath.encode(), 1 if stream else 0, buf, 512, err)
        if rc != 0:
            raise RuntimeError(err.value.decode('utf-8', errors='replace'))
        return buf.value.decode()
    ...
    def shake(self, algo, text, outlen):
        ...
        err = ctypes.create_string_buffer(1024)
        arr = (ctypes.c_uint8 * len(data))(*data)
        rc = self.dll.lab4_shake(algo.encode(), arr, len(data), outlen, buf, 512, err)
        if rc != 0:
            raise RuntimeError(err.value.decode('utf-8', errors='replace'))
        return buf.value.decode()
    ...
    def hmac(self, algo, key_hex, text):
        ...
        rc = self.dll.lab4_hmac(algo.encode(), karr, len(key), darr, len(data), buf, 512, err)
        if rc != 0:
            raise RuntimeError(err.value.decode('utf-8', errors='replace'))
        return buf.value.decode()
    ...
    def parse_x509(self, cert_path):
        ...
        err = ctypes.create_string_buffer(1024)
        rc = self.dll.lab4_parse_x509(cert_path.encode(), buf, 65536, err)
        if rc != 0:
            raise RuntimeError(err.value.decode('utf-8', errors='replace'))
    ...
    def length_ext(self, mac_hex, key_len, msg_len, append_data):
        ...
        rc = self.dll.lab4_length_ext(mac_hex.encode(), key_len, msg_len,
                                       append_data.encode(), buf, 65536, err)
        if rc != 0:
            raise RuntimeError(err.value.decode('utf-8', errors='replace'))
        return json.loads(buf.value.decode())
    ...
    def md5_demo(self, dir_path):
        buf = ctypes.create_string_buffer(65536)
        err = ctypes.create_string_buffer(1024)
        rc = self.dll.lab4_md5_demo(dir_path.encode(), buf, 65536, err)
        if rc != 0:
            raise RuntimeError(err.value.decode('utf-8', errors='replace'))
        return json.loads(buf.value.decode())
...
class Lab4GUI(QWidget):
    ...
    def create_hash_tab(self):
        ...
        layout.addWidget(QLabel("Input Text:")); layout.addWidget(self.hash_input)
        ...
        layout.addWidget(QLabel("Input File:")); layout.addLayout(file_l)
        layout.addWidget(self.hash_stream_cb)
        ...
        self.hash_output.setMaximumHeight(100); self.hash_output.setFont(self._mono_font())
    ...
    def create_hmac_tab(self):
        ...
        self.hmac_input.setPlaceholderText("Enter message...")
        self.hmac_input.setMaximumHeight(100)
        layout.addWidget(QLabel("Message:")); layout.addWidget(self.hmac_input)
        ...
        self.hmac_output.setMaximumHeight(100); self.hmac_output.setFont(self._mono_font())
    ...
    def create_shake_tab(self):
        ...
        self.shake_input = QTextEdit()
        self.shake_input.setPlaceholderText("Enter text...")
        self.shake_input.setMaximumHeight(100)
        layout.addWidget(QLabel("Input:")); layout.addWidget(self.shake_input)
        ...
        self.shake_output.setMaximumHeight(100); self.shake_output.setFont(self._mono_font())
    ...
    def do_shake(self):
        if not self.engine: return
        try:
            result = self.engine.shake(self.shake_algo.currentText(),
                                        self.shake_input.toPlainText(),
                                        self.shake_outlen.value())
            self.shake_output.setText(f"{self.shake_algo.currentText().upper()} ({self.shake_outlen.value()} bytes): {result}")
        except Exception as e:
